...
author: "Tyler Busby & Alyssa Walter"
format: 
  html: default
  gfm: default
...
```{r}
#| label: "load-in-issues-data"
issues <- read_csv("data/issues.csv")#read in issues
```
...
unvotes |> 
  count(country, vote) |> #counts number of votes by each country
  group_by(country) |> #group by country
  mutate(total = sum(n)) |> #create variable total by adding the number of votes 
  mutate(prop_vote = n/total) |> #create variable proportion of votes
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #selects only countries we want
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #create a ggplot with this data
  geom_col(position = position_stack()) + #make it a stacked barplot
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #add descriptive labels
  theme_minimal() +  #set theme
  scale_fill_viridis_d(end = 0.8) + #set viridis color pallatte
  coord_flip() #flip x and y
```

> It explains the proportion of yes/no/abstain votes by each country in the G7.  
...
```{r}
#| label: "find-distinct-short-variables"
roll_calls |> 
  distinct(short)
```

> The code finds distinct values in the variable short.
...
> The short coliumn is very descriptive, bit it is a lot of information, so it is hard ti grasp. If there was a way to more generally categorize it, we would better understand the voting issues of the UN over time.
...
> There are fewer rows and columns in the issues data than the roll_calls data, meaning there are votes in the issues data that are not covered in the roll_call data.  
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") #joining by same rcid values and adding issues to the left by column
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") #joining the same rcid values and adding issues to the right by column
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") #joining roll_calls and issues by rcid by using all rcid values
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") #joining roll_calls and issues by rcid rows found in both datasets
```
...
> Right join would be the correct join to use for our data.  
...
```{r}
#| label: "right_join-conflict/weapons-lineplot"
roll_calls |> 
  ...
  mutate(issue = case_when(
    ...
    issue == "Palestinian conflict" ~ "Palestinian Conflict")) |>   #setting these  issues as the same "issues"
  ...
  count(year, issue) |> #counts number of issues in each year
  ggplot(aes(x = year, y = n, group = issue)) + #creates plot using x = year, y = n, and grouping by issue
  geom_line(aes(color = issue)) + #creates line plot, colors lines by group
  labs(x = "Year", y = "Number of Votes",
...
> These three issues seem related. What overarching factors contributed to their similar pattern?
